    ...
    def train(self, epochs=30):
        ...
        if not self._load_model():
            ...
            for name in self.classes_names:
                ...
                metrics[output_name] = [
                    tf.keras.metrics.BinaryAccuracy(name=f'accuracy_{name}'),
                    tf.keras.metrics.Precision(name=f'precision_{name}'),
                    tf.keras.metrics.Recall(name=f'recall_{name}'),
                    F1Score(name=f'f1score_{name}')
                ]
